...
export type GetRedeemConduitCallParameters = Omit<
  ...
  'outputAsset'
> & {
  /** `conduit.asset()` with no amount floor, when omitted. */
  outputAsset?: Asset
}
...
export type GetRedeemConduitCallReturnType = {
  ...
  /**
   * The `query.salt` the redeem will carry. The id exists only once the transaction lands; join an
   * indexer on this, or read the id with {@link extractQueryIds}.
   */
  querySalt: Hex
  outputAsset: Asset
}
...
/**
 * Resolves the conduit's underlying asset from chain and returns the redeem call with the salt the
 * created query will carry. Reads only; sending the call is yours.
 *
