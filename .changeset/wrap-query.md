...
read who holds the claim ([#55](https://github.com/railnetorg/railnet-sdk/pull/55)). The SDK spawned an OwnerRegistry without shipping the ABI to use it.
...
- `wrap` clears the owner and sets a token id, so a zero owner means the token holder owns the
  ...
  at once.
- No `unwrap` builder: it is keyed on `msg.sender` as the conduit namespace, so only the owning
  conduit can call it.
