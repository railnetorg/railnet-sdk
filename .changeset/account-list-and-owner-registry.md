...
OwnerRegistry records who owns a conduit's live queries and lets that owner wrap the claim into a transferable ERC-721 — `buildSpawnOwnerRegistryCall`, `predictOwnerRegistryDeployment`. It carries no access control of its own, and its spawn params declare `symbol` before `name`.
