...
| `MissingRole(role, scope, caller)` | The caller is missing a required access control role (e.g., `VEHICLE_STEAM`). |
...
| `TransferNotAllowed(from, to)` | The transfer is blocked by the current `TransferMode` setting. |
