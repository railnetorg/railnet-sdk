...
// What you collect as you go:
// extractAccessControlAddress(receipt, eacFactory) — the ExternalAccessControl address
// extractMultiVehicleContracts(receipt, multiVehicleFactory) — { multiVehicle, queryRedeemQueue,
//   queueStrategyEngine, sectorAccountingEngine, subQueryEngine, vehicleManager }
// every transaction hash, in order — a partial deployment is only resumable if you kept them
```

### Spawning the MultiVehicle itself

Step 4 of the sequence, on its own.
