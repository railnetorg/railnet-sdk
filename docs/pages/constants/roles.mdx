...
The protocol's roles, as the `keccak256` hashes `Roles.sol` declares.
...
| Constant | Description |
| --- | --- |
| `VEHICLE_PROCESS_QUEUE` | Permission to process a vehicle's queue |
...
| `CONDUIT_SET_TRANSFER_ENABLED` | Permission to enable share transfers |
...
`ROLES` and `roleName` cover the whole set, for labelling a hash read from chain or building a role
picker.
