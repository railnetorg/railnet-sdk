...
Individual constants cover the case where you know the role you want. When you need the whole set —
labelling a hash read from chain, or building a role picker — use `ROLES` and `roleName` instead of
rebuilding the list by introspecting the exports.
...
Every role, including `DEFAULT_ADMIN_ROLE`. A test asserts it stays in step with the named exports
in both directions.
