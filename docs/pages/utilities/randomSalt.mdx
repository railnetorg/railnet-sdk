...
[`prepareSpawnMultiVehicle`](/actions/prepareSpawnMultiVehicle) takes seven at once. Query salts are disposable, but
`prepareDepositConduit` and `prepareRedeemConduit` still require one — pass a fresh one per call.
