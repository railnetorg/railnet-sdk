...
Required, because the prediction takes the whole `SpawnParams` tuple. It does not move the address:
only `deploymentSalt` and the fields `ConduitFactory._getInitCode` reads (`name`, `symbol`,
`vehicle`, the four module addresses, `transferEnabled`, `initialInterceptions`) do.
