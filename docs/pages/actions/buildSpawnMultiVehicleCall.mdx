---
description: Deploys a new MultiVehicle ecosystem via the factory.
---

# buildSpawnMultiVehicleCall

Deploys a new MultiVehicle ecosystem via the factory contract. Returns the call to send. Hand it to viem's `simulateContract` then `writeContract`, or to wagmi's `useWriteContract`.

:::warning
The factory pulls an initial deposit from the caller. Approve the factory for at least
[`getInitialDepositAmount`](/actions/getInitialDepositAmount) of `asset` first, and hold at least
that much — this action sends no approval of its own. The
[Deploying a multi-vehicle](/workflows/deployingAMultiVehicle) guide gives the order.
:::

## Import

```ts
import { buildSpawnMultiVehicleCall } from '@railnetorg/railnet-sdk'
```

## Usage

```ts
import { simulateContract, writeContract } from 'viem/actions'
import { buildSpawnMultiVehicleCall } from '@railnetorg/railnet-sdk'

const { request } = await simulateContract(client, {
  ...buildSpawnMultiVehicleCall({
    factory: '0x1234...',
    asset: '0x6B175474E89094C44Da98b954EedeAC495271d0F',
    name: 'My MultiVehicle',
    symbol: 'MMV',
    accessControl: '0x...',
    queryRegistry: '0x...',
    salts: {
    multiVehicle: randomSalt(),
    queryRedeemQueue: randomSalt(),
    queueStrategyEngine: randomSalt(),
    sectorAccountingEngine: randomSalt(),
    subQueryEngine: randomSalt(),
    vehicleManager: randomSalt(),
    initialDepositQuery: randomSalt(),
    },
  }),
  account: '0xd2135CfB216b74109775236E36d4b433F1DF507B',
})

const hash = await writeContract(client, request)
```

## Returns

A prepared call: `{ address, abi, functionName, args }`.

:::info
Use [`extractMultiVehicleContracts`](/utilities/receiptHelpers#extractmultivehiclecontracts) to extract the deployed MultiVehicle contract addresses from the transaction receipt.
:::

## Parameters

### factory

- **Type:** `Address`

The address of the MultiVehicle factory contract.

### asset

- **Type:** `Address`

The address of the underlying asset.

### name

- **Type:** `string`

The name of the MultiVehicle.

### symbol

- **Type:** `string`

The symbol of the MultiVehicle token.

### accessControl

- **Type:** `Address`

The address of the access control contract.

### queryRegistry

- **Type:** `Address`

The address of the query registry contract.

### feeManager (optional)

- **Type:** `Address`

The address of the fee manager contract. Defaults to `zeroAddress`.

### modulesManager (optional)

- **Type:** `Address`

The address of the modules manager contract. Defaults to `zeroAddress`.

### forbiddenAddresses (optional)

- **Type:** `Address[]`

Optional list of addresses forbidden from interacting with the MultiVehicle.

### salts

- **Type:** `MultiVehicleSalts`

Six deployment salts, one per contract, and `initialDepositQuery` for the seed deposit's query.
Each proxy is deployed before it is initialized, so its salt alone fixes its address — generate
each with [`randomSalt`](/utilities/randomSalt) and keep them.

```ts
type MultiVehicleSalts = {
  multiVehicle: Hex
  queryRedeemQueue: Hex
  queueStrategyEngine: Hex
  sectorAccountingEngine: Hex
  subQueryEngine: Hex
  vehicleManager: Hex
  initialDepositQuery: Hex
}
```

### initialInterceptions (optional)

- **Type:** [`Interception[]`](/types/Interception) (same shape, declared inline on this action)

Optional initial interception rules for reward distribution. Defaults to `[]`.
