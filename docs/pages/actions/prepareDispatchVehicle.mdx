...
[`prepareMoveBetweenSectors`](/actions/prepareMoveBetweenSectors) this is what replaced the removed `rebalance()`:
...
```ts
import { prepareDispatchVehicle, simulateDispatchVehicle } from '@railnetorg/railnet-sdk'
```
...
import { prepareDispatchVehicle } from '@railnetorg/railnet-sdk'
...
A prepared call: `{ address, abi, functionName, args }` for `prepareDispatchVehicle`.
