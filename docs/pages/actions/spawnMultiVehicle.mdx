...
The seven deployment salts, one per contract. Required: they fix seven addresses at once, so
losing them means a spawn that fails midway cannot be retried against the same addresses.
Generate each with [`randomSalt`](/utilities/randomSalt) and keep them.
