...
`prepareDepositConduit` and `prepareRedeemConduit` emit **only** the Railnet call. The execute
actions (`depositConduit`, `redeemConduit`) also check the ERC-20 allowance and send an approval
transaction when it is insufficient — the builders do not. Sequence the approval yourself.
