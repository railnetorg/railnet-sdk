...
Every builder that takes a salt requires it. A `prepare*` is a pure function of its inputs: two
calls with the same parameters encode the same calldata, so a prepared call can be compared against
a simulation or replayed after a failure.

Generate salts with [`randomSalt`](/utilities/randomSalt), and keep the deployment ones — they fix
the deployed address, and `prepareSpawnMultiVehicle` takes seven at once.
...
random value, because a query salt is disposable and the action makes the full round trip itself.
The `spawn*` actions have no defaults at all.
