...
Salt for the spawn's initial query. Required: a `prepare*` builder draws nothing from the clock, so the same input always encodes the same calldata. Generate it with [`randomSalt`](/utilities/randomSalt).
