---
description: Estimates the output of a conduit operation.
---

# estimateConduit

Estimates the output of a conduit operation given an input asset and mode.

## Import

```ts twoslash
import { estimateConduit } from '@railnetorg/railnet-sdk'
```

## Usage

```ts twoslash
import { createPublicClient, http } from 'viem'
import { base } from 'viem/chains'
import { estimateConduit, QueryMode, EstimationType } from '@railnetorg/railnet-sdk'

const client = createPublicClient({
  chain: base,
  transport: http(),
})

const estimatedAsset = await estimateConduit(client, {
  conduit: '0xA0Cf798816D4b9b9866b5330EEa46a18382f251e',
  asset: { asset: '0x6B175474E89094C44Da98b954EedeAC495271d0F', value: 1000000000000000000n },
  mode: QueryMode.DEPOSIT,
  estimationType: EstimationType.OUTPUT,
})
```

## Returns

[`Asset`](/types/Asset)

```ts
type Asset = {
  asset: Address
  value: bigint
}
```

A deposit estimate is in conduit shares, after conduit fees. A redeem estimate is in the
underlying asset. A `minOutput` floor takes [`estimateVehicle`](/actions/estimateVehicle), in
vehicle shares.

## Parameters

### conduit

- **Type:** `Address`

```ts
const estimatedAsset = await estimateConduit(client, {
  conduit: '0xA0Cf798816D4b9b9866b5330EEa46a18382f251e', // [!code focus]
  asset: { ... },
  mode: QueryMode.DEPOSIT,
  estimationType: EstimationType.OUTPUT,
})
```

### asset

- **Type:** [`Asset`](/types/Asset)

```ts
const estimatedAsset = await estimateConduit(client, {
  conduit: '0x...',
  asset: { asset: '0x...', value: 1000000000000000000n }, // [!code focus]
  mode: QueryMode.DEPOSIT,
  estimationType: EstimationType.OUTPUT,
})
```

### mode

- **Type:** [`QueryMode`](/types/Enums#querymode)

```ts
const estimatedAsset = await estimateConduit(client, {
  conduit: '0x...',
  asset: { ... },
  mode: QueryMode.DEPOSIT, // [!code focus]
  estimationType: EstimationType.OUTPUT,
})
```

### estimationType

- **Type:** [`EstimationType`](/types/Enums#estimationtype)

```ts
const estimatedAsset = await estimateConduit(client, {
  conduit: '0x...',
  asset: { ... },
  mode: QueryMode.DEPOSIT,
  estimationType: EstimationType.OUTPUT, // [!code focus]
})
```
