...
Estimates the output of a conduit operation given an input asset and mode. A deposit estimate is in
conduit shares, after conduit fees. A redeem estimate is in the underlying asset. A `minOutput` floor
takes [`estimateVehicle`](/actions/estimateVehicle), in vehicle shares.
...
const estimatedAsset = await estimateConduit(client, {
  conduit: '0x...',
  asset: { ... },
  mode: QueryMode.DEPOSIT, // [!code focus]
  estimationType: EstimationType.OUTPUT,
})
```
