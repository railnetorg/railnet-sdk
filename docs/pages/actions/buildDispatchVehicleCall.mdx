...
bubbles the vehicle's state up unchanged, so it is any member of `QueryState`, not just `SETTLED` or
`PROCESSING`. `SETTLED` and `REJECTED` are terminal and a rejected query can never be progressed;
on any other state, keep the `query` — progressing an in-flight one later requires the exact struct.
