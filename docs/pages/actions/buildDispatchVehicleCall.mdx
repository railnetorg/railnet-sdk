...
MultiVehicle's SectorAccountingEngine. Reallocating between two vehicles is a redeem dispatch
followed by a deposit dispatch, in [Rebalancing between vehicles](/guides/rebalancingBetweenVehicles).
...
Syrup) it leaves the query short of settlement, and the funds are not where you want them until the
query is progressed. Call `simulateDispatchVehicle` first if the difference matters — which it does
whenever a second leg depends on the first.
:::
...
`{ query: Query; state: QueryState }` for `simulateDispatchVehicle`, which sends nothing. The engine
bubbles the vehicle's state up unchanged, so it is any member of `QueryState`. `SETTLED` and
`REJECTED` are terminal and a rejected query can never be progressed; on any other state, keep the
`query` — progressing an in-flight one later requires the exact struct.
