...
4. Grants `MULTI_VEHICLE_SET_VEHICLE_AUTHORIZATION` role on the SectorAccountingEngine (SAE) to the admin.
5. Grants `MULTI_VEHICLE_SET_QUEUES` role on the QueueStrategyEngine (QSE) to the admin.
6. Grants `VEHICLE_STEAM` role on the vehicle to the MultiVehicle contract.
7. Grants `VEHICLE_STEAM` role on the vehicle to the SectorAccountingEngine.
8. Grants `VEHICLE_STEAM` role on the vehicle to the SubQueryEngine.
9. Authorizes the vehicle in the VehicleRegistry.
10. Sets the initial queues on the QSE.
